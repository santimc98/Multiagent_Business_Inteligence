...
def test_reviewer_generation_config_includes_response_schema_when_enabled(monkeypatch):
    monkeypatch.setenv("REVIEWER_USE_RESPONSE_SCHEMA", "1")
    agent = ReviewerAgent(api_key=None)

    cfg = agent._generation_config_for_review(["runtime_success", "contract_required_artifacts_missing"])
    ...
    assert failed_items.get("enum") == ["runtime_success", "contract_required_artifacts_missing"]


def test_reviewer_generate_gemini_retries_without_response_schema(monkeypatch):
    monkeypatch.setenv("REVIEWER_USE_RESPONSE_SCHEMA", "1")
    agent = ReviewerAgent(api_key=None)
    ...
    content, used_config = agent._generate_gemini_json(
        "prompt",
        generation_config=agent._generation_config_for_review(["runtime_success"]),
    )
